...
app.config["SECRET_KEY"] = os.getenv("TH_SECRET")
...
class Answers(db.Model):
    ...
    def __repr__(self):
        return (
            "Level: " + self.level + " Team: " + self.team + " Answer: " + self.answer
        )
...
def puzzle():
    ...
    if request.method == "POST":
        ...
        if current_level >= TOTAL_QUIZ:
            ...
            try:
                # only update the level is answer matches.
                submitted = Answers(
                                level_name=str(current_user.level_completed ),
                ...
                if current_puzzle and current_puzzle.answer.strip().lower() == answer.strip().lower():
                    ...
                    stmt = (
                            ...
                            .values(level_completed=User.level_completed + 1,
                                    last_time=datetime.utcnow().replace(microsecond=0))
                            )
...
def team_reg():
    if current_user.is_authenticated and current_user.role == "ADMIN":
        if request.method == "GET":
            ...
            user = User(
                id=teamid, name=teamname, pwd=password_hash, token=token, role="TEAM"
            )
            db.session.add(user)
            db.session.commit()
    ...
    if current_user.is_authenticated and current_user.role == "ADMIN":
        ...
        if request.method == "GET":
            ...
            if level == "" and teamname != "":
                answer_list = Answers.query.filter_by(team=teamname).order_by(
                    Answers.id.desc()
                )
            elif teamname == "" and level != "":
                level_int = level
                answer_list = Answers.query.filter_by(level_name=level_int).order_by(
                    Answers.id.desc()
                )
